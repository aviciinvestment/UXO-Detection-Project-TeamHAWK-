...
**Input:** RGB image frame from device camera  
...
**Dataset:** CTX-UXO – A Comprehensive Dataset for Detection and Identification of Unexploded Ordnances  
**Authors:** Gheorghe Marian Craioveanu, Grigore Stamatescu  
...
Preprocessing is handled internally by the training pipeline:
...
- Optimizer & LR schedule: default YOLO configuration  
...
Evaluation was performed on the validation and test sets.
...
- Reduced sensitivity to small or ambiguous objects  
...
The trained model was exported to TensorFlow Lite using **INT8 quantization**.

**Pipeline:**
YOLO (.pt) → TensorFlow → TFLite (.tflite, INT8)
...
3. Run inference via TFLite interpreter  
4. Decode output tensors  
5. Render bounding boxes and labels  
